...
class SteppedProgress extends StatefulWidget {
  ...
  const SteppedProgress(
      ...
      this.padding = const EdgeInsets.only(left: 20.0, right: 20.0)})
      : super(key: key);
...
class _SteppedProgressState extends State<SteppedProgress> {
  ...
  void initState() {
    ...
    for (var index = 0; index < temp.length; index++) {
      // same length as widgets.steps
      stepKeys[temp[index]] = GlobalKey<_SteppedProgressState>();
      plottedToReal[temp[index]] =
          widget.steps.indexOf(widget.steps[index]) + 1;
      realToPlotted[index + 1] = temp[index];
      offstageKeys[index] =
          GlobalKey<_SteppedProgressState>(); // first index = 0
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      offstageKeys.forEach((index, globalKey) {
        ...
        Widget child = Positioned(
            top: _offset.dy,
            left: _offset.dx,
            child: Text(widget.steps[index], style: widget.textStyle));
      ...
      setState(() {
        ...
        rightPadding =
            _getWidgetSize(offstageKeys[widget.steps.length - 1]!).width / 2;
      });
    });
  }
  ...
  int _getCurrentStep() => widget.currentStep == 1
      ? 1
